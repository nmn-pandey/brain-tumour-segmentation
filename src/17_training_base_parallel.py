...
output_file = open("17_output.txt", "w")
error_file = open("17_errors.txt", "w")
...
criterion = CombinedLoss(num_classes=4, class_weights=class_weights_list, device_num = gpus[0]).to(device)
...
model_path = '../saved_models/17_best_model_base.pt'
...
for epoch in range(1000):
    ...
    for batch_num, (imgs, masks) in enumerate(train_data_gen):
        ...
        with amp.autocast():
            logits = model(imgs) # Probability as Model Outputs - REQUIRED FOR CROSS ENTROPY LOSS
            outputs = torch.argmax(logits, dim=1) # To remove one-hot encoding
        ...
        print("Batch:", batch_num,", Combined Loss:", loss.item())#,", Dice Loss:", dice_loss(masks, outputs).mean().item())
    ...
    if (epoch + 1) % 50 == 0:
        ...
        with torch.no_grad():  # Disable gradients for validation
            for val_batch_num, (val_imgs, val_masks) in enumerate(val_data_gen):
                ...
                val_loss += criterion(val_logits, val_masks).mean().item()
    ...
    results_df.to_csv('17_training_results.csv', index=False)
    ...
    if epoch_loss < best_loss:
        ...
        if isinstance(model, nn.DataParallel):
            ...
            torch.save(model.module.state_dict(), '../saved_models/17_best_model_base.pt')
        else:
            # Save the state dictionary as-is if the model is not wrapped with nn.DataParallel
            print("Saving without nn.DataParallel")
            torch.save(model.state_dict(), '../saved_models/17_best_model_base.pt')

# Save the final model
if isinstance(model, nn.DataParallel):
    print("Saving final model with nn.DataParallel")
    torch.save(model.module.state_dict(), '../saved_models/17_final_model_base.pt')
else:
    print("Saving final model without nn.DataParallel")
    torch.save(model.state_dict(), '../saved_models/17_final_model_base.pt')
