...
from models import Swin_AR2B_DeepSup_UNet
...
output_file = open("09_output.txt", "w")
error_file = open("09_errors.txt", "w")
...
# Define Batch Size
BATCH_SIZE = 4 #12
accumulation_steps = 30 #5

# Specify GPU ids to use
gpus = [1, 4, 5, 6] #0, 4, 7] # [3, 5, 6, 7] 
...
model_path = '../saved_models/09_best_model_swin.pt'
...
for epoch in range(1000):
    ...
    results_df.to_csv('09_training_results.csv', index=False)
    ...
    if epoch_loss < best_loss:
        ...
        if isinstance(model, nn.DataParallel):
            ...
            torch.save(model.module.state_dict(), '../saved_models/09_best_model_swin.pt')
        else:
            # Save the state dictionary as-is if the model is not wrapped with nn.DataParallel
            print("Saving without nn.DataParallel")
            torch.save(model.state_dict(), '../saved_models/09_best_model_swin.pt')

# Save the final model
if isinstance(model, nn.DataParallel):
    print("Saving final model with nn.DataParallel")
    torch.save(model.module.state_dict(), '../saved_models/09_final_model_swin.pt')
else:
    print("Saving final model without nn.DataParallel")
    torch.save(model.state_dict(), '../saved_models/09_final_model_swin.pt')
