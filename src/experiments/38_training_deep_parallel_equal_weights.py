...
output_file = open("38_output.txt", "w")
error_file = open("38_errors.txt", "w")
...
"""
class_weights = {0: 0.25, 1: 0.25, 2: 0.25, 3: 0.255}
...
model_path = '../saved_models/38_best_model_deep.pt'
...
for epoch in range(1000):
    ...
    results_df.to_csv('38_training_results.csv', index=False)
    ...
    if epoch_loss < best_loss:
        ...
        if isinstance(model, nn.DataParallel):
            ...
            torch.save(model.module.state_dict(), '../saved_models/38_best_model_deep.pt')
        else:
            # Save the state dictionary as-is if the model is not wrapped with nn.DataParallel
            print("Saving without nn.DataParallel")
            torch.save(model.state_dict(), '../saved_models/38_best_model_deep.pt')

# Save the final model
if isinstance(model, nn.DataParallel):
    print("Saving final model with nn.DataParallel")
    torch.save(model.module.state_dict(), '../saved_models/38_final_model_deep.pt')
else:
    print("Saving final model without nn.DataParallel")
    torch.save(model.state_dict(), '../saved_models/38_final_model_deep.pt')
