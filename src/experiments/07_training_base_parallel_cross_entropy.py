...
output_file = open("07_output.txt", "w")
error_file = open("07_errors.txt", "w")
...
model_path = '../saved_models/07_best_model_base.pt'
...
for epoch in range(1000):
    ...
    for batch_num, (imgs, masks) in enumerate(train_data_gen):
        ...
        print("Batch:", batch_num,", Cross Entropy Loss:", loss.item(),", Dice Loss:", dice_loss(masks, outputs).mean().item())
    ...
    results_df.to_csv('07_training_results.csv', index=False)
    ...
    if epoch_loss < best_loss:
        ...
        if isinstance(model, nn.DataParallel):
            ...
            torch.save(model.module.state_dict(), '../saved_models/07_best_model_base.pt')
        else:
            # Save the state dictionary as-is if the model is not wrapped with nn.DataParallel
            print("Saving without nn.DataParallel")
            torch.save(model.state_dict(), '../saved_models/07_best_model_base.pt')

# Save the final model
if isinstance(model, nn.DataParallel):
    print("Saving final model with nn.DataParallel")
    torch.save(model.module.state_dict(), '../saved_models/07_final_model_base.pt')
else:
    print("Saving final model without nn.DataParallel")
    torch.save(model.state_dict(), '../saved_models/07_final_model_base.pt')
