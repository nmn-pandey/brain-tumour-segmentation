...
output_file = open("28_output.txt", "w")
error_file = open("28_errors.txt", "w")
...
model_path = '../saved_models/28_best_model_deep.pt'
...
for epoch in range(1000):
    ...
    results_df.to_csv('28_training_results.csv', index=False)
    ...
    if epoch_loss < best_loss:
        ...
        if isinstance(model, nn.DataParallel):
            ...
            torch.save(model.module.state_dict(), '../saved_models/28_best_model_deep.pt')
        else:
            # Save the state dictionary as-is if the model is not wrapped with nn.DataParallel
            print("Saving without nn.DataParallel")
            torch.save(model.state_dict(), '../saved_models/28_best_model_deep.pt')

# Save the final model
if isinstance(model, nn.DataParallel):
    print("Saving final model with nn.DataParallel")
    torch.save(model.module.state_dict(), '../saved_models/28_final_model_deep.pt')
else:
    print("Saving final model without nn.DataParallel")
    torch.save(model.state_dict(), '../saved_models/28_final_model_deep.pt')
